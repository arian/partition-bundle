...
function partitioner(map, firstFile) {
  ...
  function depsBelongTo(deps, file, topLevel) {
    forOwn(deps, function(_dep) {
      var dep = modulesByID[_dep];
      if (!dep) {
        throw new Error("Could not lookup dependency " + _dep);
      }
      var id = dep.dedupe || dep.id;
      var belong = moduleBelongsTo[id];
      var count = belong[file] = topLevel ? Infinity : (belong[file] || 0) + 1;
      console.log(id, moduleBelongsTo);
      // stop at 3, otherwise it might be a cyclic dependency
      if (dep.deps && (count <= 3 || topLevel)) depsBelongTo(dep.deps, file);
    });
  }
